...
// Налаштування для обробки статичних файлів (CSS, JS, зображення)
...
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'Home', 'Home.html'));  // Головна сторінка з папки 'HHome'
});
  ...
  db.query(query, [email], (err, results) => {
    ...
    bcrypt.compare(password, user.password, (err, isMatch) => {
      ...
      res.cookie('userToken', token, {
        httpOnly: true,  // доступно лише через HTTP
        secure: true,    // працює тільки через HTTPS
        maxAge: 3600000, // термін дії 1 година
      });
...
app.get('/', (req, res) => {
  ...
  jwt.verify(token, 'secretKey', (err, decoded) => {
    ...
    res.sendFile(path.join(__dirname, 'HOme', 'indexhome.html'));
  });
});
